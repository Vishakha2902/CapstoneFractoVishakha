...
//                 UserId = userId,
//                 Score = dto.Score
//                 // no CreatedAt in your model; omitting
//             });
//             await _db.SaveChangesAsync();

//             // Average as decimal, then round/store
...
//             // If Doctor has decimal? Rating, persist it
//             if (doctor.GetType().GetProperty("Rating") is not null)
//             {
//                 doctor.Rating = Math.Round(avgDec, 2);
//                 await _db.SaveChangesAsync();
//             }

//             return Ok(new { doctorId = dto.DoctorId, average = Math.Round((double)avgDec, 2) });
//         }
...
//             var avg = (double)(avgDec ?? 0m);
//             return Ok(new { doctorId, average = Math.Round(avg, 2) });
//         }
...
//                 .Where(r => r.DoctorId == doctorId)
//                 .OrderByDescending(r => r.Id)      // no CreatedAt => sort by Id desc
...
//             // Average as DECIMAL end-to-end
//             var avgDec = await _db.Ratings
//                 .Where(r => r.DoctorId == dto.DoctorId)
//                 .Select(r => (decimal)r.Score) // works whether Score is int or decimal
//                 .AverageAsync();

//             // Persist to Doctor.Rating if present
//             if (doctor.GetType().GetProperty("Rating") is not null)
//             {
//                 doctor.Rating = Math.Round(avgDec, 2);
//                 await _db.SaveChangesAsync();
...
//             // Return DECIMAL (no double conversion)
//             return Ok(new { doctorId = dto.DoctorId, average = Math.Round(avgDec, 2) });
//         }
